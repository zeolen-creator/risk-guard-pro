...
export default function EditAssessmentPage() {
  ...
  useEffect(() => {
    if (assessment && !initialized) {
      ...
      setSelectedHazards(assessment.selected_hazards || []);
      setProbabilities(assessment.probabilities || {});
      setImpacts(assessment.impacts || {});
      
      // Determine which steps are completed
      const completed: number[] = [];
      if ((assessment.selected_hazards || []).length > 0) completed.push(1);
      if (Object.keys(assessment.probabilities || {}).length > 0) completed.push(2);
      if (Object.keys(assessment.impacts || {}).length > 0) completed.push(3);
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          <div className="flex gap-2">
            {currentStep < 4 ? (
              <Button
                onClick={handleNext}
                disabled={!canProceed() || updateAssessment.isPending}
              >
